...
int main(int argc, char** argv) {
    ...
    if (!instrument_manager.load_from_json(filepath)) {
        ...
        // continue: historical manager demo can still run
    }
    ...
    if (!instruments.empty()) {
        ...
        long long now = 0; // demo: no timestamps available; skip actual aggregation if no numeric timestamps
        auto rows = hdm.get_by_instrument(sample);
        if (rows && !rows->empty()) {
            // attempt to aggregate using the first and last timestamps if present
