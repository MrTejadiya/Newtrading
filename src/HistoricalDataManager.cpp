    ...
    for (auto& entry : fs::directory_iterator(dir)) {
        ...
        try {
            if (p.extension() == ".json") {
                ...
                json j;
                f >> j;
                // If top-level is an object with instrument info, try to extract key
                auto ik = find_instrument_in_json(j);
                string instrument = ik.value_or(instrument_from_filename(p.stem().string()));
                if (j.is_array()) {
